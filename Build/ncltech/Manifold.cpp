...
{	
	...
	if (constraintMass > 0.0f)
		...
		// Add after float jn=
		// and before jn = jn / constraintMass ;
		...
		if (frictionalMass > 0.0f)
			...
			float frictionCoef =
				...
				// Add after float jt=
				// and before jt = jt / frictionalMass ;

				// Stop Friction from ever being more than frictionCoef * normal
			    // resolution impulse . Similar to above for SumImpulseContact , except
				// for friction the direction of friction is potentially changing each
				// time we solve the contact (as other objects are bouncing this
				// object in different directions ). So instead of a scalar , we need
			    // to clamp the total tangential impulse vector to the normal
			    // resolution impulse . This way , we can solve friction in any
				// direction during the solving step without having to precompute and
				// check a single tangent vector .
